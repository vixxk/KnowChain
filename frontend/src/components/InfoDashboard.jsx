...
function Skeleton({ className = "h-6 w-20" }) {
...
export default function InfoDashboard() {
	...
	const secReport = metrics?.security_report || {};
	...
	const traceList = (() => {
		const remoteTraces = obs.recent_traces || [];
		const combined = [...userTraces];
		for (const rt of remoteTraces) {
			if (!combined.some(t => t.id === rt.id || (t.query && t.query === rt.query))) {
				combined.push(rt);
			}
		}
		return combined.length ? combined : remoteTraces;
	})();
			...
			<div className="flex-1 overflow-y-auto p-3 sm:p-6 lg:p-8 text-[#eef0f3] space-y-4 sm:space-y-6 pb-24 sm:pb-8 relative z-10 scrollbar-hide">
				<div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
							...
							<span className="font-semibold uppercase tracking-wider text-[11px]">
								{isLoading ? (
									<span className="inline-flex items-center gap-2">
										<span>Auditing Golden Benchmark Suite</span>
										<Skeleton className="h-3 w-28" />
									</span>
								) : (
									"Evaluation Suite Verified • 8/8 Golden Benchmark Samples Audited"
								)}
							</span>
						</div>
							...
							<span className="inline-flex items-center gap-1.5">
								...
								{isLoading ? (
									...
									<strong className="text-[#60a5fa]">
										{ragas.retrieval?.recall_at_k !== undefined ? `${((ragas.retrieval.recall_at_k > 1 ? ragas.retrieval.recall_at_k / 100 : ragas.retrieval.recall_at_k) * 100).toFixed(1)}%` : "---"}
									</strong>
								)}
							</span>
							<span className="inline-flex items-center gap-1.5">
								...
								{isLoading ? (
									...
									<strong className="text-[#34d399]">{secReport.summary ? '100% SECURE' : 'SECURE'}</strong>
								)}
							</span>
						</div>
					</div>
					...
					<div className="hero-blue-glow bg-[#101216]/75 backdrop-blur-md border border-[#1f2229]/80 p-4 sm:p-8 rounded-xl flex flex-col md:flex-row items-center justify-between gap-6">
						...
						<div className="flex flex-col items-center justify-center p-4 sm:p-6 bg-[#08090b]/80 border border-[#2a2d36] rounded-xl shrink-0 min-w-[200px] min-h-[96px]">
							{isLoading ? (
								<Skeleton className="h-10 sm:h-12 w-32 sm:w-36 my-1" />
							) : (
								<span className="text-4xl sm:text-5xl font-black font-mono tracking-tight text-[#60a5fa]">
									{((ragas.overall_ragas_score ?? 0) * 100).toFixed(1)}%
								</span>
							)}
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-2 hover:border-[#2a2d36] transition-all">
							...
							<div className="min-h-[28px] sm:min-h-[36px] flex items-center">
								{isLoading ? (
									...
									<div className={`text-xl sm:text-3xl font-bold ${getMetricColorClass(ragas.retrieval?.recall_at_k)}`}>
										{ragas.retrieval?.recall_at_k !== undefined ? ((ragas.retrieval.recall_at_k > 1 ? ragas.retrieval.recall_at_k / 100 : ragas.retrieval.recall_at_k)).toFixed(3) : "---"}
									</div>
								)}
							</div>
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-2 hover:border-[#2a2d36] transition-all">
							...
							<div className="min-h-[28px] sm:min-h-[36px] flex items-center">
								{isLoading ? (
									...
									<div className={`text-xl sm:text-3xl font-bold ${getMetricColorClass(ragas.retrieval?.precision_at_k)}`}>
										{ragas.retrieval?.precision_at_k !== undefined ? ((ragas.retrieval.precision_at_k > 1 ? ragas.retrieval.precision_at_k / 100 : ragas.retrieval.precision_at_k)).toFixed(3) : "---"}
									</div>
								)}
							</div>
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-2 hover:border-[#2a2d36] transition-all">
							...
							<div className="min-h-[28px] sm:min-h-[36px] flex items-center">
								{isLoading ? (
									...
									<div className={`text-xl sm:text-3xl font-bold ${getMetricColorClass(ragas.generator?.faithfulness)}`}>
										{ragas.generator?.faithfulness !== undefined ? ((ragas.generator.faithfulness > 1 ? ragas.generator.faithfulness / 100 : ragas.generator.faithfulness)).toFixed(3) : "---"}
									</div>
								)}
							</div>
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-2 hover:border-[#2a2d36] transition-all">
							...
							<div className="min-h-[28px] sm:min-h-[36px] flex items-center">
								{isLoading ? (
									...
									<div className={`text-xl sm:text-3xl font-bold ${getMetricColorClass(ragas.generator?.answer_relevancy)}`}>
										{ragas.generator?.answer_relevancy !== undefined ? ((ragas.generator.answer_relevancy > 1 ? ragas.generator.answer_relevancy / 100 : ragas.generator.answer_relevancy)).toFixed(3) : "---"}
									</div>
								)}
							</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 font-mono">
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-1.5">
							<div className="flex items-center gap-2 text-[#6b7280] text-[10px] uppercase font-semibold">
								...
								<span>Pipeline Latency (avg)</span>
							</div>
							<div className="min-h-[30px] flex items-center">
								{isLoading ? (
									...
									<div className="text-xl sm:text-2xl font-bold text-[#eef0f3]">
										{obs.tracing?.latency_avg_ms || obs.latency?.p95_ms || "0"} <span className="text-xs font-normal text-[#6b7280]">ms</span>
									</div>
								)}
							</div>
						</div>
						...
						<div className="bg-[#101216]/70 backdrop-blur-md border border-[#1f2229]/80 p-3.5 sm:p-5 rounded-xl space-y-1.5">
							...
							{isLoading ? (
								<div className="space-y-1.5">
									...
									<div><Skeleton className="h-3 w-40" /></div>
								</div>
							) : (
								<>
									<div className="text-xl sm:text-2xl font-bold text-[#eef0f3]">
										<span className="text-[#60a5fa]">{obs.cost?.total_tokens?.toLocaleString() || "0"}</span>
									</div>
									<div className="text-[10px] text-[#6b7280]">
										Prompt: {obs.cost?.prompt_tokens?.toLocaleString() || "0"} • Comp: {obs.cost?.completion_tokens?.toLocaleString() || "0"}
									</div>
								</>
							)}
						</div>
							...
							<div className="min-h-[30px] flex items-center">
								{isLoading ? (
									...
									<div className="text-xl sm:text-2xl font-bold text-[#34d399]">
										${obs.cost?.daily_cost_usd !== undefined ? obs.cost.daily_cost_usd : (obs.cost?.total_cost_usd || "0.0000")}
									</div>
								)}
							</div>
							<div className="text-[10px] text-[#6b7280]">
								Fireworks AI DeepSeek-V4 & Nomic Embeddings
							</div>
						</div>
					</div>
				...
				<div className="bg-[#101216]/75 backdrop-blur-md border border-[#1f2229]/80 rounded-xl overflow-hidden space-y-0">
					...
					<div className="block sm:hidden divide-y divide-[#1f2229] p-3 space-y-3">
						{isLoading && traceList.length === 0 ? (
							Array.from({ length: 3 }).map((_, idx) => (
								<div key={idx} className="bg-[#0c0e12]/60 backdrop-blur-sm border border-[#1f2229]/80 p-3 rounded-lg space-y-2 font-mono text-xs">
							...
							<div className="p-4 text-center text-xs text-[#6b7280] font-mono">
								No execution traces recorded yet. Query in Workspace to trace real executions.
							</div>
						) : (
							traceList.map((t, idx) => (
								<div 
						...
						<table className="w-full text-left text-xs font-mono text-[#9ca3af]">
							...
							<tbody className="divide-y divide-[#1f2229] bg-[#101216]/40">
								{isLoading && traceList.length === 0 ? (
									Array.from({ length: 3 }).map((_, idx) => (
										<tr key={idx}>
											<td className="p-3"><Skeleton className="h-3.5 w-16" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-48" /></td>
											<td className="p-3"><Skeleton className="h-4 w-32" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-16" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-14" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-12" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-16" /></td>
										</tr>
									))
								) : traceList.length === 0 ? (
									<tr>
										<td colSpan={7} className="p-6 text-center text-[#6b7280] font-mono text-xs">
											No execution traces recorded yet. Query in Workspace to trace real executions.
										</td>
									</tr>
								) : (
				...
				<div className="bg-[#101216]/75 backdrop-blur-md border border-[#1f2229]/80 rounded-xl overflow-hidden space-y-3 p-3.5 sm:p-5">
					<div className="border-b border-[#1f2229] pb-2.5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
						<div>
							<h2 className="text-xs sm:text-sm font-semibold text-[#eef0f3] font-mono">NeMo Security Guardrails & Safety Matrix</h2>
							<p className="text-[10px] sm:text-xs text-[#6b7280] mt-0.5 font-mono">Real-time prompt injections, PII leakage, context audit, and exfiltration defense</p>
						</div>
						{secReport.live_queries_audited !== undefined && (
							<span className="px-2 py-0.5 bg-[#08090b] border border-[#2a2d36] text-[10px] sm:text-[11px] font-mono rounded text-[#34d399] font-semibold flex items-center gap-1.5 w-fit">
								<span className="w-1.5 h-1.5 rounded-full bg-[#34d399] animate-pulse"></span>
								Active Telemetry ({secReport.live_queries_audited} live chats monitored)
							</span>
						)}
					</div>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3 font-mono">
						{isLoading ? (
							Array.from({ length: 8 }).map((_, i) => (
								<div key={i} className="bg-[#0c0e12]/60 backdrop-blur-sm border border-[#1f2229]/80 p-3 rounded-lg flex items-center justify-between">
									<div className="space-y-1.5 flex-1 pr-2">
										<Skeleton className="h-3.5 w-28 block" />
							...
							Object.entries(security && Object.keys(security).length > 0 ? security : {}).map(([key, item]) => {
								const formattedName = key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
								const isPassed = item.status === "PASSED" || item.status === "SECURE";
								return (
									<div key={key} className="bg-[#0c0e12]/60 backdrop-blur-sm border border-[#1f2229]/80 p-3 rounded-lg flex items-center justify-between hover:border-[#2a2d36] transition-all">
										<div className="space-y-0.5">
											...
											<div className="text-[10px] text-[#6b7280]">
												...
												{item.total_audited ? ` (${item.total_audited} tests)` : ''}
											</div>
										</div>
										<div className={`flex items-center gap-1.5 text-xs ${isPassed ? 'text-[#34d399]' : 'text-[#f87171]'}`}>
											<span className={`w-1.5 h-1.5 rounded-full ${isPassed ? 'bg-[#34d399]' : 'bg-[#f87171]'}`}></span>
				...
				<div className="bg-[#101216]/75 backdrop-blur-md border border-[#1f2229]/80 rounded-xl overflow-hidden space-y-0">
					<div className="px-3.5 sm:px-5 py-3 border-b border-[#1f2229] flex items-center justify-between">
						...
						<span className="px-2 py-0.5 bg-[#08090b] border border-[#2a2d36] text-[11px] font-mono rounded text-[#60a5fa] font-semibold min-w-[70px] text-center inline-block">
							{isLoading && sampleList.length === 0 ? (
					...
					<div className="block sm:hidden divide-y divide-[#1f2229] p-3 space-y-3">
						{isLoading && sampleList.length === 0 ? (
							Array.from({ length: 3 }).map((_, idx) => (
								<div key={idx} className="bg-[#0c0e12]/60 backdrop-blur-sm border border-[#1f2229]/80 p-3.5 rounded-lg space-y-2.5 font-mono text-xs">
									<div className="flex items-center justify-between">
										<Skeleton className="h-4 w-20" />
										<Skeleton className="h-4 w-16" />
									</div>
							...
							<div className="p-4 text-center text-xs text-[#6b7280] font-mono">
								No benchmark evaluation samples available.
							</div>
						) : (
							sampleList.map((s) => (
								<div 
							...
							<tbody className="divide-y divide-[#1f2229] bg-[#101216]/40">
								{isLoading && sampleList.length === 0 ? (
									Array.from({ length: 5 }).map((_, idx) => (
										<tr key={idx}>
											<td className="p-3"><Skeleton className="h-3.5 w-16" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-20" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-48" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-36" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-36" /></td>
											<td className="p-3"><Skeleton className="h-3.5 w-12" /></td>
											<td className="p-3"><Skeleton className="h-4 w-14 rounded-full" /></td>
											<td className="p-3"><Skeleton className="h-5 w-16 rounded" /></td>
										</tr>
									))
								) : sampleList.length === 0 ? (
									<tr>
										<td colSpan={8} className="p-6 text-center text-[#6b7280] font-mono text-xs">
											No benchmark evaluation samples available.
										</td>
									</tr>
								) : (
